  ...
  return new Promise(function (resolve, reject) {
    ...
    categories.forEach(function (cat, index, arr) {
      ...
      var placeUrl = "https://graph.facebook.com/" + self.version + "/search" +
        ...
        "&fields=" + fields.join(",") +
        "&access_token=" + self.accessToken;
      urls.push(getPlaces(placeUrl));
    });
    Promise.all(urls).then(function (responseBody) {
      //Get places as specified for each category
      // });
      // getPlaces(urls).then(function (responseBody) {
      var ids = [],
        tempArray = [];
      responseBody.forEach(function (request, index, arr) {
        console.log("Unfiltered Places found: " + request.data.length);
        request.data.forEach(function (data, index, arr) {
          places.push(data);
        });
      ...
      places.forEach(function (idObj, index, arr) {
        if (i == 123) {
          tempArray.push("172955189439262");
        }
        i++;
        // idObj.forEach(function (place, index, arr) {
        idObj.events = [];
