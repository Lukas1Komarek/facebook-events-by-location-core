...
EventSearch.prototype.fillEventObject = function (event, venueId, venue, eventResultObj, categoryList) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (!self.latitude || !self.longitude) {
            ...
            getPlaces(placeUrl).then(function (responseBody) {
                ...
                promises['likes'] = [];
                promises['events'] = [];
                function fillEventObject(event, venueId, venue, eventResultObj, categoryList) {
                    eventResultObj.id = event.id;
                    eventResultObj.name = event.name;
                    eventResultObj.type = event.type;
                    eventResultObj.coverPicture = (event.cover ? event.cover.source : null);
                    ...
                    Object.getOwnPropertyNames(resObj).forEach(function (venueId, index, array) {
                        ...
                        if (venue.events && venue.events.data.length > 0) {
                            venue.events.data.forEach(function (event, index, array) {
                                ...
                                // if owner is page request likes (new api request necessary)
                                if (event.is_page_owned) {
                                    eventResultObj.ownedByPage = true;
                                    var pageId = event.owner.id;
                                    eventResultObj.pageId = pageId;
                                    eventResultObj = fillEventObject(event, venueId, venue, eventResultObj, categoryList);
                                    var requestPageUrl = pageUrl + pageId + "/" +
                                        "?fields=fan_count" +
                                        "&access_token=" + self.accessToken;
                                    promises['likes'].push(rp.get(requestPageUrl));
                                    allEvents.push(eventResultObj);
                                } else {
                                    eventResultObj.ownedByPage = false;
                                    eventResultObj = fillEventObject(event, venueId, venue, eventResultObj, categoryList);
                                    allEvents.push(eventResultObj);
                                }
                ...
                return promises;
            }).then(function (promisesLikes) {
                //Run Graph API requests in parallel
                return Promise.all(promisesLikes['likes'])
            }).then(function (results) {
                var promises = [];
                results.forEach(function (resStr, index, arr) {
                    var resPage = JSON.parse(resStr);
                    allEvents.forEach(function (event, index, arr) {
                        if (event.pageId == resPage.id) {
                            event.pageLikes = resPage.fan_count;
                        }
                    });
                });
                return promises;
                //Run Graph API requests in parallel
            }).then(function (promisisesEventsWithLikes) {
                //Run Graph API requests in parallel
                return Promise.all(promisisesEventsWithLikes)
            }).then(function (results) {
                // console.log(results);
