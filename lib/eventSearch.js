    ...
    return new Promise(function (resolve, reject) {
        ...
        if (!self.latitude || !self.longitude) {
            ...
            getPlaces(placeUrl).then(function (responseBody) {
                ...
                function fillEventObject(event, venueId, venue, eventResultObj) {
                                ...
                                venue.events.data.forEach(function (event, index, array) {
                                    ...
                                    if (event.is_page_owned) {
                                        ...
                                        var requestPageUrl = pageUrl + pageId + "/" +
                                            "?fields=fan_count" +
                                            "&access_token=" + self.accessToken;
                                        console.log(requestPageUrl);
                                        rp.get(requestPageUrl).then(function (htmlString) {
                                            var resPage = JSON.parse(htmlString);
                                            eventResultObj.likes = resPage.fan_count;
                                            // console.log(resPage);
                                            eventResultObj = fillEventObject(event, venueId, venue, eventResultObj);
                                            events.push(eventResultObj);
                                            eventsCount++;
                                            resolve(eventResultObj);
